...
2. **Copy template**: `ui_component_template.md` → `docs/components/[ComponentName]_doc.md`
...
- ✅ Reusable across multiple pages (Button, Card, Table, Modal)
- ✅ Business-critical (enrollment forms, course catalogs)
- ✅ Complex behavior (controlled inputs, data tables, async operations)
- ✅ Frequently modified (high churn = needs documentation)

**Skip components that are**:
- ❌ Trivial wrappers with no logic (just styling)
- ❌ One-off page-specific components (unless very complex)
- ❌ Auto-generated by tools (form builders, code generators)
...
✅ Props interface (if in separate file)
...
```
✅ Component tests (shows usage patterns)
   📁 src/components/common/Button.test.tsx

✅ Parent component that uses this component
...
✅ API client file (for Backend Integration section)
   📁 src/services/apiClient.ts
   (CRITICAL: Enables AI to map UI → API endpoints)

✅ Backend files (for FULL Backend Integration automation)
   📁 ../backend/TrainingTracker.Api/Controllers/UsersController.cs
   📁 ../backend/TrainingTracker.Api/Services/UserService.cs
   📁 ../backend/TrainingTracker.Api/Repositories/UserRepository.cs
   (OPTIONAL: Only if backend code is in workspace/monorepo)

✅ Design system documentation
   📁 docs/design-system/button-spec.md
```
...
│   ├── courses/
│   │   ├── components/
│   │   │   ├── CourseCard.tsx          ← Document this
│   │   │   ├── CourseCard.module.css   ← Attach
│   │   │   └── CourseCard.test.tsx     ← Attach
│   │   ├── hooks/
│   │   │   └── useCourses.ts           ← Attach (if component uses it)
│   │   ├── services/
│   │   │   └── courseService.ts        ← Attach (if component calls it)
│   │   └── types/
│   │       └── Course.ts                ← Attach (data model)
```
...
│   └── src/
│       └── Button/
│           ├── Button.tsx              ← Document this
│           ├── Button.module.css       ← Attach
│           ├── Button.test.tsx         ← Attach
│           ├── Button.stories.tsx      ← Attach
│           └── Button.types.ts         ← Attach
└── app/
    └── src/
        └── pages/
            └── CourseCatalog.tsx       ← Attach (usage example)
```
...
5. **Attach files using @ mentions**:
   ...
   **Recommended attachments**:
   ```
   @ui_component_template.md        ← Template (MUST)
   @Button.tsx                      ← Component implementation (MUST)
   @Button.module.css               ← Styles (SHOULD)
   @Button.test.tsx                 ← Tests (SHOULD)
   @EnrollmentForm.tsx              ← Usage example (OPTIONAL)
   ```
...
Please:
1. Fill in all 🤖 (AI-generated) sections based on the component code
...
6. Leave all ❓ (human input needed) sections for me to fill in
7. Use the exact structure from the template
8. **INCLUDE METADATA SECTION WITH TAGS** - Critical for automated consolidation

Focus on:
- **Metadata**: Component type, repository, file path, last updated, TAGS
...
- **Backend Integration**: Map UI → API Client → Endpoint → Service → Database
- Accessibility (ARIA attributes, keyboard support)

**CRITICAL: Tagging Requirements**:
- **MUST include Metadata section with tags** at the top of documentation
- **Tags format**: Space-separated hashtags in kebab-case (e.g., #course-catalog #button #ui-component)
- **Tag categories to include**:
  1. Feature Domain: Which business features use this component? (e.g., #course-enrollment #user-profile)
  2. Cross-Cutting Concerns: What system-wide capabilities? (e.g., #authentication #error-handling #accessibility)
  3. Component Type: #ui-component (always), plus specific type (#button, #form, #modal, #table, etc.)
  4. Priority: #core-feature, #important, or #nice-to-have
- **Use 3-8 tags per component** (be selective but comprehensive)
- **Reference**: See TAGGING_STRATEGY_TAXONOMY.md for complete tag list

**CRITICAL: Backend Integration Mapping**:
- **AI can auto-generate**: UI Layer → API Client Layer → Backend Endpoint (by analyzing component code and apiClient.ts)
- **AI CANNOT auto-generate** (unless backend in workspace): Backend Service → Database Table
- **For full automation**: Attach backend files (controller, service, repository) if available in workspace
- **For partial automation**: AI generates UI→Endpoint, then manually add Service→Database from backend docs
...
**Tags**: #feature-tag1 #feature-tag2 #cross-cutting-tag #ui-component #component-type-tag #priority-tag
```

**Tag examples by component type**:
- Button: `#ui-component #button #core-feature`
- CourseCard: `#course-catalog #ui-component #important`
- EnrollmentForm: `#enrollment #enrollment-validation #ui-component #form #core-feature`
- LoginForm: `#user-authentication #authentication #ui-component #form #core-feature`
- ErrorBoundary: `#error-handling #ui-component #important`

**How to determine tags**:
1. Analyze component name and purpose → feature domain tags
2. Check props for feature-specific data (course, user, enrollment) → feature tags
3. Check for authentication requirements → #authentication
4. Check for error handling → #error-handling
5. Identify component type (button, form, modal, table, etc.) → component type tags
6. Always include #ui-component
7. Consider business criticality → #core-feature, #important, or #nice-to-have

**How to generate Backend Integration section**:
1. AI will auto-detect API calls in component code (e.g., `usersApi.create()`)
2. AI will trace to apiClient.ts to find HTTP endpoints (e.g., `POST /api/users`)
3. **If backend code is in workspace**: Attach backend controller/service files → AI generates full chain
4. **If backend code is separate**: AI generates UI→Endpoint, leave Service→Database for manual fill-in

**CRITICAL: Generate Developer Action Summary**:
At the END of the documentation, create a "Developer Action Items" section with:
1. **High Priority** (❗ Must complete before PR): Business context, security, critical gaps
2. **Medium Priority** (📋 Should complete soon): UX details, performance notes, testing
3. **Low Priority** (💡 Nice to have): Future enhancements, edge cases, optimizations
- List each ❓ section that needs human input with line/section reference
- Estimate time needed for each priority level
- Group related items together

Attached files for context (Frontend):
- [ComponentName].tsx (implementation) - REQUIRED
- [ComponentName].module.css (styles) - REQUIRED if exists
- [ComponentName].test.tsx (tests showing usage) - RECOMMENDED
- [ParentComponent].tsx (real-world usage) - RECOMMENDED
- apiClient.ts (API integration) - REQUIRED for Backend Integration section
- ui_component_template.md (template to follow) - REQUIRED

Attached files for context (Backend - OPTIONAL for full automation):
- [Controller].cs (backend controller) - OPTIONAL: Enables AI to auto-generate Service names
- [Service].cs (backend service) - OPTIONAL: Enables AI to auto-generate Database table names
- [Repository].cs (data access) - OPTIONAL: Confirms database table mappings
```
...
Please:
1. Fill in all 🤖 sections based on the Button.tsx code
...
6. Leave ❓ sections for me to fill in
7. Keep the exact template structure
8. Include Metadata section with appropriate tags
...
**Tags for Button component**:
- Feature domain: General UI (used across multiple features)
- Component type: #button #ui-component
- Priority: #core-feature (used everywhere)
- Example: `#ui-component #button #core-feature`
...
- ✅ Project structure and existing documentation
...
2. **Component Overview**: What it is, when to use, visual purpose
3. **Props API**: Complete table with prop name, type, required/optional, default, description
4. **Visual States**: All states (default, hover, active, disabled, loading, error, focus)
5. **Usage Examples**: At least 3 examples (basic, with state, with API/real data)
6. **Backend Integration**: Map UI component → API client → endpoint → service → database (if applicable)
7. **Accessibility**: ARIA attributes, keyboard navigation, screen reader support
8. **Component Behavior**: Controlled vs uncontrolled, side effects, event handling
9. **Styling**: CSS modules, variants, customization options
...
✅ Generate examples from test files and parent components
✅ Document all visual states and variants
✅ **INCLUDE METADATA SECTION WITH TAGS** - Critical for automated consolidation
✅ **GENERATE BACKEND INTEGRATION MAP** - Auto-generate UI→Endpoint, flag Service→Database if backend not in workspace

**Tagging instructions**:
- **Feature Domain Tags**: Identify which business features use this component (e.g., #course-catalog, #enrollment, #user-profile)
- **Cross-Cutting Concern Tags**: Identify system-wide capabilities (e.g., #authentication, #error-handling, #accessibility)
- **Component Type Tags**: Always include #ui-component, plus specific type (#button, #form, #modal, #table, #navigation, etc.)
- **Priority Tag**: Include #core-feature, #important, or #nice-to-have based on business criticality

**Backend Integration instructions**:
- **Step 1**: Detect all API calls in component code (e.g., `usersApi.getAll()`, `coursesApi.create()`)
- **Step 2**: Trace each call to apiClient.ts to find HTTP method + endpoint (e.g., `GET /api/users`)
- **Step 3**: If backend files attached to workspace → Extract service names and database tables
- **Step 4**: If backend NOT in workspace → Mark Service and Database columns with ❓ placeholder
- **Step 5**: Add links to related backend documentation (API controller, service, database table docs)
...
- LoginForm: `#user-authentication #authentication #ui-component #form #core-feature`
- ErrorBoundary: `#error-handling #ui-component #important`
- Modal: `#ui-component #modal #important`

**How to determine tags**:
1. Analyze component name and purpose → feature domain tags
2. Check props for feature-specific data (course, user, enrollment) → feature tags
3. Check for authentication/authorization → #authentication #authorization
4. Check for error handling → #error-handling
5. Check for accessibility features → #accessibility (if notable implementation)
6. Identify component type → #button, #form, #modal, #table, etc.
7. Always include #ui-component
8. Consider business criticality → #core-feature, #important, or #nice-to-have
- Target component: [COMPONENT_NAME] (file: [PATH_TO_COMPONENT])
- Output location: docs/components/[ComponentName]_doc.md
...
✅ Mark AI-generated content with 🤖
✅ Mark sections needing human input with ❓
✅ Extract props from TypeScript interface into Props API table
✅ Document all variants from union types ('primary' | 'secondary' | 'danger')
✅ Extract ARIA attributes from JSX
✅ Focus on WHAT (observable from code) - mark 🤖
✅ Flag WHY questions (business context, UX rationale) - mark ❓
...
- ❓ Visual appearance descriptions (what does it look like?)
- ❓ UX rationale (why this interaction pattern?)
- ❓ Business context (why was this component created?)
- ❓ Accessibility testing results (tested with NVDA?)
- ❓ Performance considerations (render time measurements)
- ❓ Browser compatibility issues (Safari bugs, etc.)
- ❓ Design decisions (why these specific variants?)

**Props API table format**:
| Prop | Type | Required | Default | Description |
|------|------|----------|---------|-------------|
| 🤖 `propName` | `string` | Yes/No | `'value'` | 🤖 Purpose of prop |
...
**CRITICAL: Generate Developer Action Summary**:
At the END of the documentation, create a "Developer Action Items" section with:
1. **High Priority** (❗ Must complete before PR): Business context, security concerns, critical unknowns
2. **Medium Priority** (📋 Should complete soon): UX details, accessibility notes, integration considerations
3. **Low Priority** (💡 Nice to have): Future enhancements, edge cases, nice-to-have details
- List each ❓ section that needs human input with section reference
- Estimate time needed for each priority level (e.g., "~10 min", "~30 min")
- Group related items together for efficient completion
...
- ✅ Extracting props from TypeScript interfaces
- ✅ Generating props table with types and defaults
- ✅ Identifying variants from union types (`'primary' | 'secondary' | 'danger'`)
- ✅ Listing imported dependencies
- ✅ Extracting ARIA attributes from JSX
- ✅ Generating basic code examples
- ✅ Documenting component structure (hooks, state, effects)

**Copilot struggles with**:
- ❌ Visual descriptions ("what does it look like?")
- ❌ UX rationale ("why this interaction pattern?")
- ❌ Accessibility testing ("tested with NVDA?")
- ❌ Business context ("why was this component created?")
- ❌ Performance measurements ("render time: X ms")
- ❌ Browser compatibility issues ("Safari bug with...")
...
// ❌ AI might say variant is required (it's optional)
// ❌ AI might omit default value for variant
// ❌ AI might incorrectly describe onClick callback
...
**Common AI mistakes**:
```tsx
// ❌ AI might generate incomplete example
...
- ❓ "Save" (blue background #007bff, white text, rounded corners)
- ❓ "Cancel" (white background, blue border, blue text)
- ❓ "Delete" (red background #dc3545, white text, rounded corners)
...
🤖 A reusable button component with multiple variants for different action types.
...
❓ **Measured with React DevTools Profiler** (Chrome 119, 100 renders):
...
❓ **Bundle size** (measured with webpack-bundle-analyzer):
- Component code: 2.1 KB (minified)
- CSS Modules: 0.8 KB (minified)
- Total: 2.9 KB (gzipped: 1.1 KB)

❓ **Optimization applied**:
- ✅ React.memo: Not needed (simple component, fast render)
- ✅ useCallback: Not needed (no child components re-rendering)
- ✅ CSS Modules: Scoped styles, no global CSS pollution

❓ **Bottlenecks**: None identified. Component is lightweight and fast.
```
...
|-------|--------|------------|----------|
| ❓ Safari 14: Focus outline not visible on click | Low - keyboard users see outline | Use `:focus-visible` instead of `:focus` | Ticket #456 |
| ❓ IE11: CSS Grid not supported | N/A - IE11 not supported | None | N/A |
...
| ✅ Firefox | 120+ | ✅ Full | None |
| ✅ Safari | 17+ | ✅ Full | None |
| ✅ Edge | 119+ | ✅ Full | None |
| ⚠️ Safari | 14-16 | ⚠️ Partial | Focus outline issue (low impact) |
| ❌ IE 11 | - | ❌ Not supported | CSS Grid not supported |
...
- ❓ **Q**: Should this button support icon-only variants?
  - **Ask**: UX team (Jane Doe)
  - **Deadline**: Before v2.0 release

- ❓ **Q**: What's the maximum button text length before wrapping?
  - **Test**: Create example with long text
  - **Document**: Add to "Limitations" section

- ❓ **Q**: Is this component used in mobile app?
  - **Ask**: Mobile team (John Smith)
  - **Action**: If yes, add mobile-specific docs

### Documentation Gaps

- ❓ **Gap**: Missing Storybook stories for all variants
  - **Action**: Create Button.stories.tsx
  - **Owner**: You
  - **Deadline**: This sprint

- ❓ **Gap**: No visual regression tests
  - **Action**: Set up Chromatic, add snapshots
  - **Owner**: QA team
  - **Deadline**: Next sprint

- ❓ **Gap**: Screen reader testing incomplete (iOS/Android not tested)
...
- ❓ **Debt**: CSS Modules use magic numbers for spacing
  - **Refactor**: Extract to design tokens
  - **Ticket**: #892
  - **Priority**: Low

- ❓ **Debt**: No TypeScript generics for polymorphic "as" prop
...
**Quality checks**:
- [ ] All 🤖 sections filled in (no placeholder text)
- [ ] ❓ sections enhanced with human context
...
- ✅ Flexible structure (use only parts you need)
- ✅ Clear visual hierarchy in code
- ✅ Easy to extend (add new sub-components)
- ✅ Type-safe (TypeScript validates sub-component props)
```
...
// User authenticated → renders DashboardPage
```
...
├── CourseSearch (search/filter)
├── CoursePagination (pagination)
└── CourseActions (bulk actions)
```
...
- [ ] All 🤖 sections generated by AI
- [ ] All ❓ sections enhanced by human
...
- ✅ Props API (X props documented)
- ✅ Visual states (X states documented)
- ✅ Usage examples (X examples)
- ✅ Accessibility (WCAG Level AA, keyboard nav, screen reader tested)
...
- ❓ **Q**: Why was this component created? What problem does it solve?
...
**Status**: ⚠️ **DEPRECATED** - Use `Button` component instead  
...
2. `isLoading` prop → renamed to `loading`
...
⚠️ **Warning**: This component is under active development. Props and behavior may change without notice. Do not use in production yet.
...
❌ **Not applicable**: This component renders once on page load and never updates. No performance optimization needed.
```
...
✅ **Fully compatible**: Component uses only standard HTML/CSS. No browser-specific features. No compatibility issues.
```
...
1. ⏱️ **2 min** - Identify component, gather files
2. ⏱️ **1 min** - Copy template, prepare workspace
3. ⏱️ **10 min** - Use Copilot to generate baseline
4. ⏱️ **3 min** - Review and correct AI output
5. ⏱️ **10 min** - Enhance with human context
6. ⏱️ **3 min** - Final validation and checklist

**Total: 20-30 minutes per component**

**Key success factors**:
- ✅ Attach right files to Copilot (component + tests + usage)
- ✅ Use standard prompt (copy-paste from this guide)
- ✅ Always enhance AI output (don't just accept it)
- ✅ Focus on visual examples and accessibility
