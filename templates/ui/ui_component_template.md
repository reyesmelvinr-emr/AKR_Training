...
**🤖 AI can auto-generate**: UI Layer → API Client Layer → Backend Endpoint  
**❓ Human must provide**: Backend Service → Database Table (unless backend code is in workspace)

---

#### Full-Stack Visibility (Monorepo or Multi-Workspace)

🤖 **If backend code is in your workspace**, AI can auto-generate the complete chain:

| UI Layer | → | API Client Layer | → | Backend Endpoint | → | Backend Service | → | Database Table |
|----------|---|------------------|---|------------------|---|-----------------|---|----------------|
| 🤖 [ComponentName] | → | 🤖 `apiClient.method()` | → | 🤖 `HTTP_METHOD /api/resource` | → | 🤖 `ServiceName.MethodName()` | → | 🤖 `schema.TableName` |

**How to enable full auto-generation**:
```
When documenting with Copilot, attach backend files:
- Backend controller (e.g., UsersController.cs)
- Backend service (e.g., UserService.cs)
- Database repository or schema files

AI will trace the full path from UI → Database automatically.
```

---

#### Frontend-Only Visibility (Separate Repos)

🤖 **If backend code is NOT in workspace**, AI generates partial chain:

| UI Layer | → | API Client Layer | → | Backend Endpoint | → | Backend Service | → | Database Table |
|----------|---|------------------|---|------------------|---|-----------------|---|----------------|
| 🤖 [ComponentName] | → | 🤖 `apiClient.method()` | → | 🤖 `HTTP_METHOD /api/resource` | → | ❓ [Fill from backend docs] | → | ❓ [Fill from DB docs] |

**After AI generation, manually add**:
- Backend Service names from backend documentation
- Database Table names from database documentation
- Links to related backend/database docs

---

#### Example: Users Component (Full Chain)
...
**Cross-reference backend documentation** (add links manually or let AI auto-link if files exist):

- **API Endpoints**: See [API Controller Documentation](../../../backend/docs/api/[Controller]_api.md) for detailed endpoint specifications
- **Business Logic**: See [Service Documentation](../../../backend/docs/services/[Service]_doc.md) for service layer logic and business rules
- **Database Schema**: See [Table Documentation](../../../POC_SpecKitProj/docs/tables/[Table]_doc.md) for table structure and constraints

---

### Integration Details

❓ **Human input needed** (AI cannot infer these from code):

**Authentication & Authorization**:
- Is authentication required for these endpoints?
- How are tokens/credentials passed (headers, cookies)?
- What roles/permissions are needed?

**Performance & Reliability**:
- Rate limiting: Are there request limits?
- Caching: Should responses be cached? For how long?
- Retry logic: Should failed requests retry automatically?
- Timeout: What's the expected response time?

**Error Handling**:
- What HTTP error codes can be returned?
- How should the UI handle specific errors (401, 403, 429, 500)?
- Are there user-friendly error messages to display?

**Development & Testing**:
- Offline fallback: Mock data? Local storage? Disabled features?
- API versioning: Which API version does this component use?
- CORS: Any cross-origin concerns?
