...
1. **Metadata** (component type, repository, file path, last updated, TAGS)
2. Service Identification (name, namespace, file location, complexity)
3. Quick Reference (TL;DR: what it does, when to use, watch out for)
4. What & Why (purpose, capabilities, scope boundaries)
5. How It Works (primary operation with step-by-step flow)
6. Business Rules (table format: Rule ID, Description, Why It Exists, Since When)
7. Architecture (Dependencies and Consumers tables with failure modes)
8. Data Operations (Reads From, Writes To, Side Effects)
9. Questions & Gaps (flag unknowns, technical debt, magic numbers)
...
- **MUST include Metadata section with tags** at the top of documentation
- **Tags format**: Space-separated hashtags in kebab-case (e.g., #enrollment #validation #service)
- **Tag categories to include**:
  1. Feature Domain: Which business features does this service implement? (e.g., #enrollment #course-management)
  2. Cross-Cutting Concerns: What system-wide capabilities? (e.g., #authentication #audit-logging #caching)
  3. Component Type: #service (always include for services)
  4. Priority: #core-feature, #important, or #nice-to-have
- **Use 3-8 tags per service** (be selective but comprehensive)
- **Reference**: See TAGGING_STRATEGY_TAXONOMY.md for complete tag list
...
**Tags**: #enrollment #enrollment-validation #prerequisite-validation #authentication #service #core-feature
```

**Tag examples by service type**:
- EnrollmentService: `#enrollment #enrollment-validation #prerequisite-validation #authentication #audit-logging #service #core-feature`
- CourseService: `#course-catalog #course-management #country-based-access #service #core-feature`
- UserService: `#user-profile #user-authentication #password-management #service #core-feature`
- NotificationService: `#notifications #email #service #important`
...
**Data Operations section format**:
**Reads From table**:
| Database Object | Purpose | Business Context | Performance Notes |
|-----------------|---------|------------------|-------------------|
| 🤖 `schema.TableName` | 🤖 What data retrieved | ❓ Why needed? | 🤖 Query pattern ❓ Performance concerns |

**Writes To table**:
| Database Object | Purpose | Business Context | Performance Notes |
|-----------------|---------|------------------|-------------------|
| 🤖 `schema.TableName` | 🤖 INSERT/UPDATE/DELETE | ❓ What business event? | 🤖 Transaction scope ❓ Volume/indexing |

**Calls table** (stored procedures/functions):
| Database Object | Purpose | Business Context | Performance Notes |
|-----------------|---------|------------------|-------------------|
| 🤖 `usp_ProcName` | 🤖 What it does | ❓ Why stored proc vs service? | ❓ Execution time, timeout |

**Side Effects table**:
| Database Object | Trigger/Effect | Business Context | Impact |
|-----------------|----------------|------------------|--------|
| 🤖 Table/System | 🤖 What happens | ❓ Why this side effect? | ❓ Blocking? Async? Failure handling? |

**Extract from code**:
- Method signatures and parameters
- Dependencies from constructor injection
- Database tables/repositories from method calls (for Data Operations - Reads/Writes)
- Stored procedure calls from ADO.NET, Dapper, or EF Core raw SQL
- Exception types from throw statements
- Validation logic for business rules
- Call flow from method implementations
- Transaction scopes (TransactionScope, BeginTransaction calls)
...
- **TAGS**: Which features does this service implement? What cross-cutting concerns apply?
- **Database operations**: N+1 query risks, missing indexes, transaction scope concerns
- **Stored procedures**: Execution time, why stored proc vs service logic

**Generate appropriate tags**:
- Analyze service purpose and methods to identify feature domains
- Check for authentication/authorization logic → #authentication #authorization
- Check for logging/audit calls → #audit-logging
- Check for caching → #caching
- Identify related features from service name and business rules
- Include #service and appropriate priority tag (#core-feature, #important, #nice-to-have)
- Format: Space-separated hashtags (e.g., #enrollment #validation #service #core-feature)

**CRITICAL: Generate Developer Action Summary**:
At the END of the documentation, create a "Developer Action Items" section with:
1. **High Priority** (❗ Must complete before PR): Business rationale, security concerns, critical unknowns
2. **Medium Priority** (📋 Should complete soon): Performance thresholds, error handling details, integration notes
3. **Low Priority** (💡 Nice to have): Future refactoring, optimization ideas, edge cases
- List each ❓ section that needs human input with section reference
- Estimate time needed for each priority level (e.g., "~5 min", "~15 min")
- Group related items by stakeholder (Product Owner, Tech Lead, etc.)
...
- **Add context for better tagging**: "This service implements [Feature X] and [Feature Y]"
...
1. **Metadata**: Component type, repository, file path, last updated, TAGS
2. **Service Identification**: Namespace, file location, complexity (Simple/Medium/Complex)
3. **Quick Reference**: What it does (1-2 sentences), when to use, watch out for
4. **What & Why**: Purpose, capabilities, scope boundaries (what it does NOT do)
5. **How It Works**: Primary operation with step-by-step flow in text-based boxes
6. **Business Rules**: Table with BR-[SVC]-### format
7. **Architecture**: Dependencies table (what service needs), Consumers table (who uses it)
8. **Data Operations**: Reads From, Writes To, Calls (Stored Procedures), Side Effects tables with performance notes
9. **Questions & Gaps**: Flag unknowns, magic numbers, technical debt
...
✅ **INCLUDE METADATA SECTION WITH TAGS** - Critical for automated consolidation
✅ **DOCUMENT DATABASE OPERATIONS** - Include performance notes, transaction scope, N+1 risks

**Tagging instructions**:
- **Feature Domain Tags**: Identify which business features this service implements (e.g., #enrollment, #course-management, #user-profile)
- **Cross-Cutting Concern Tags**: Identify system-wide capabilities (e.g., #authentication, #audit-logging, #caching, #error-handling)
- **Component Type Tag**: Always include #service for services
- **Priority Tag**: Include #core-feature, #important, or #nice-to-have based on business criticality
...
**Component Type**: Service  
**Repository**: training-tracker-api  
**File Path**: `TrainingTracker.Api/Services/[ServiceName].cs`  
**Last Updated**: [Today's Date]  
**Tags**: #feature-tag1 #feature-tag2 #cross-cutting-tag #service #priority-tag
```

**Examples of proper tagging**:
- EnrollmentService: `#enrollment #enrollment-validation #prerequisite-validation #authentication #audit-logging #service #core-feature`
- CourseService: `#course-catalog #course-management #country-based-access #caching #service #core-feature`
- UserService: `#user-profile #user-authentication #password-management #authentication #service #core-feature`
- NotificationService: `#notifications #email #service #important`

**How to determine tags**:
1. Analyze service name and purpose → feature domain tags
2. Check for auth/authorization logic → #authentication #authorization
3. Check for logging/audit calls → #audit-logging
4. Check for caching logic → #caching
5. Review business rules to identify related features
6. Consider business criticality → #core-feature, #important, or #nice-to-have
...
- Database operations from repository method calls (for Reads From, Writes To tables)
- Stored procedure calls from ADO.NET, Dapper, or EF raw SQL (for Calls table)
- Transaction scopes (TransactionScope, BeginTransaction) for performance notes
...
- ❓ Database performance: N+1 query risks, missing indexes, high-volume operations
- ❓ Why stored procedures vs service logic? Business rules complexity?
...
**Data operations format (ENHANCED)**:
