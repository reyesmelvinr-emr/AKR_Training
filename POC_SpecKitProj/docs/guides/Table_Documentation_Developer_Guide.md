...
2. Table identification (schema, object type, last updated)
3. Purpose (what this table stores, business purpose)
4. Columns (all columns with generic types + native types)
5. Constraints (check, unique, foreign keys in plain English)
6. Business Rules (BR-TABLENAME-### format)
7. Related Objects (views, stored procedures, triggers, referenced by)
...
- **Tags format**: Space-separated hashtags in kebab-case (e.g., #user-profile #authentication)
- **Tag categories to include**:
  1. Feature Domain: Which business features use this table? (e.g., #course-enrollment #user-profile)
  2. Cross-Cutting Concerns: What system-wide capabilities? (e.g., #authentication #audit-logging)
  3. Component Type: #table (always include for tables)
  4. Priority: #core-feature, #important, or #nice-to-have
- **Use 3-8 tags per table** (be selective but comprehensive)
...
**File Path**: `schema/tables/Courses.sql`  
**Last Updated**: 2025-11-12  
**Tags**: #course-catalog #course-management #table #core-feature
```

**How to identify tags**:
1. **Feature Domain**: What features use this table? Check feature docs, ask PO/Tech Lead
2. **Cross-Cutting**: Does it handle auth? logging? caching? validation? encryption?
3. **Component Type**: Always #table for database tables
4. **Priority**: Core business function? Important feature? Nice-to-have?

**Tag examples by table type**:
- Users table: `#user-profile #authentication #audit-logging #table #core-feature`
- Courses table: `#course-catalog #course-management #table #core-feature`
- Enrollments table: `#enrollment #enrollment-validation #audit-logging #table #core-feature`
- CourseCategories table: `#course-catalog #table #nice-to-have`
...
- **TAGS**: Which features use this table? What cross-cutting concerns apply?

**Generate appropriate tags**:
- Analyze table purpose and columns to identify feature domains
- Check for authentication columns (UserId, CreatedBy) → #authentication
- Check for timestamp columns (CreatedDate, ModifiedDate) → #audit-logging
- Identify related features from table name and relationships
- Include #table and appropriate priority tag (#core-feature, #important, #nice-to-have)
- Format: Space-separated hashtags (e.g., #enrollment #validation #table #core-feature)

**CRITICAL: Generate Developer Action Summary**:
At the END of the documentation, create a "Developer Action Items" section with:
1. **High Priority** (❗ Must complete before PR): Business purpose, constraint rationale, critical unknowns
2. **Medium Priority** (📋 Should complete soon): Business rules context, data retention policies, migration history
3. **Low Priority** (💡 Nice to have): Related object documentation, optimization notes, historical context
- List each ❓ section that needs human input with section reference
- Estimate time needed for each priority level (e.g., "~5 min", "~10 min")
- Group related items by stakeholder (DBA, Product Owner, etc.)
...
- **Add context for better tagging**: "This table is used by [Feature X] and [Feature Y]"
...
1. **Metadata**: Component type, repository, file path, last updated, TAGS
2. **Table identification**: Schema, object type, last updated (use today's date)
3. **Purpose**: What this table stores and its business purpose (1-3 sentences)
4. **Columns**: All columns using format:
   - `ColumnName` (GenericType, Required/Nullable, default: value) - Description (native: DBSpecificType)
5. **Constraints**: Check, unique, foreign keys translated to plain English
6. **Business Rules**: Using BR-TABLENAME-### format
7. **Related Objects**: Views, stored procedures, triggers that use this table
...
✅ **INCLUDE METADATA SECTION WITH TAGS** - This is critical for automated consolidation
✅ Tags must be space-separated hashtags in kebab-case format
✅ Include 3-8 relevant tags covering: feature domain, cross-cutting concerns, component type, priority

**Tagging instructions**:
- **Feature Domain Tags**: Identify which business features use this table (e.g., #user-profile, #course-enrollment, #enrollment-validation)
- **Cross-Cutting Concern Tags**: Identify system-wide capabilities (e.g., #authentication, #audit-logging, #caching, #error-handling)
- **Component Type Tag**: Always include #table for database tables
...
**Component Type**: Table  
**Repository**: training-tracker-database  
**File Path**: `schema/tables/[TableName].sql`  
**Last Updated**: [Today's Date]  
**Tags**: #feature-tag1 #feature-tag2 #cross-cutting-tag #table #priority-tag
```

**Examples of proper tagging**:
- Users table: `#user-profile #authentication #audit-logging #table #core-feature`
- Courses table: `#course-catalog #course-management #country-based-access #table #core-feature`
- Enrollments table: `#enrollment #enrollment-validation #prerequisite-validation #audit-logging #table #core-feature`

**How to determine tags**:
1. Analyze table name and purpose → feature domain tags
2. Check for auth-related columns (UserId, CreatedBy, etc.) → #authentication
3. Check for timestamp columns (CreatedDate, ModifiedDate) → #audit-logging
4. Review foreign keys to identify related features
5. Consider business criticality → #core-feature, #important, or #nice-to-have
